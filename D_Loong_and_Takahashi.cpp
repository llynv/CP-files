...
void solve()
{
    ...
    for (int i = 0; i < n / 2; ++i) {
        ...
        for (int j = n-even-1; j <= even; ++j) {
            a[even][j] = to_string(num);
            num++;
        }
        for (int j = even - 1; j >= n-even-1; --j) {
            a[j][even] = to_string(num);
            num++;
        }
        for (int j = even - 1; j >= n-even-1; --j) {
            a[n-even-1][j] = to_string(num);
            num++;
        }
        for (int j = n-even; j < even; ++j) {
            a[j][n-even-1] = to_string(num);
            num++;
        }
