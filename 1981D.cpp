...
void solve()
{
    int n;
    cin >> n;
    auto gen = [&] (int x, int j, vector<int> a, bool& isOk, auto&& gen) {
        if (j == n) {
            map<ii, int> mp;
            // if (isOk) return;
            for (int i = 1; i < n; ++i) {
                if (mp.count({min(a[i], a[i-1]), max(a[i], a[i-1])})) return;
                mp[{min(a[i], a[i-1]), max(a[i], a[i-1])}]++;
            }
            dbg(a);
            isOk = true;
            return;
