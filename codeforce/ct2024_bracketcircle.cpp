...
struct PolyHash{
    ...
    uint64_t concat_back_to_front (int l1, int r1, int l2, int r2){
        return (modmul(get_hash(l1, r1), base_pow[r2 - l2 + 1]) + get_hash(l2, r2)) % mod;
    }
};
...
void solve()
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (cnt == 0) {
            mp[H.concat_back_to_front(i+1, n-1, 0, i)]++;
        }
    }
    dbg(mp);
    cout << mp.size() << '\n';
