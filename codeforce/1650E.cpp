...
void solve()
{
    ...
    int mxDist = 0, pos;
    for (int i = 1; i <= n; ++i) {
        cur = min(cur, a[i] - a[i-1] - 1);
        if (a[i]-a[i-1]-1 >= mxDist) {
            mxDist = a[i]-a[i-1]-1;
            pos = i;
        }
    ...
    for (int i = 1; i < n; ++i) {
        ...
        int cur1 = (a[i+1] - a[i-1] - 2) / 2;
        int cur2 = min(max((mxDist - 1) / 2, m - a[n] - 1), a[i+1] - a[i-1] - 1);

        // dbg(s);
        int tmp = INF;
        if (!s.empty()) tmp = *s.begin();
        ans = max(ans, min(max(cur1, cur2), tmp));
        // dbg(i, cur1, cur2, tmp);
        s.insert(a[i]-a[i-1]-1);
        s.insert(a[i+1]-a[i]-1);
    }
    s.erase(s.find(a[n]-a[n-1]-1));
    ans = max(ans, min(m - a[n-1] - 1, *s.begin()));
    // dbg(pos, mxDist, ans);
    cout << ans << endl;
}
