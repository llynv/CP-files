...
void solve()
{
   ...
   for (int i = 0; i < n; ++i) {
      string s;
      cin >> s;
      adj[i] = make_pair(stoll(s.substr(0, sz(s) - 1)), s.back());
   }
   map<int, Bignum> mp;
   ...
   for (int i = 0; i < n; ++i) {
      ...
      while (!vst[u]) {
         vst[u] = 1;
         path[i].push_back(adj[u].first);
         if (adj[u].second == '*') {
            cur *= Bignum(adj[u].first);
         } else if (adj[u].second == '+') {
            cur += Bignum(adj[u].first);
         } else if (adj[u].second == '-') {
            cur -= Bignum(adj[u].first);
         } else {
