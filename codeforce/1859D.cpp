...
void solve()
{
    ...
    while (j < n) {
        cur = max(cur, seg[j].se);
        while (j < n - 1 && cur >= seg[j+1].fi) {
            cur = max(cur, seg[j+1].se);
            ++j;
        }

        points.push_back({seg[i].fi, cur});
    ...
    for (auto &c : p) {
        int pos = upper_bound(all(points), ii(c, INF)) - points.begin();
        pos--;

        if (pos < 0 || points[pos].se < c) {
            cout << c << " ";
            continue;
        }
        ...
        if (points[pos].fi <= c && c <= points[pos].se) {
            cout << points[pos].se << " ";
            continue;
        }
