...
void solve()
{
    ...
    long long l = -1e16, r = 1e16, pos = 0;
    Bignum res;
    for (long long i = 0; i < n; ++i) {
        Bignum a1 = a[i], c1 = c[i];
        a1.sign = 0;
        c1.sign = 0;
        res += a1 * c1;
    }
    ...
    if (res > f(Bignum(pos-1)) || res > f(Bignum(pos+1))) return void(cout << "-inf\n");
    cout << res.to_str() << '\n';
}
