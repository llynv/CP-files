...
void solve()
{
    ...
    vector<int> a(n);
    for (auto &c : a) cin >> c;
    vector<int> even(n+1), odd(n+1), pre(n+1);
    for (int i = 1; i <= n; ++i) {
        even[i] = even[i-1] + (a[i-1] % 2 == 0);
        odd[i] = odd[i-1] + (a[i-1] % 2 != 0);
        pre[i] = pre[i-1] + a[i-1];
    }
    vector<ii> pos;
    int l = 0;
    for (int i = 0; i < n - 1; ++i) {
        if (!(((abs(a[i]) ^ abs(a[i+1])) & 1) > 0)) {
            // dbg(a[i], a[i+1]);
            pos.push_back({l, i});
            l = i+1;
        }
    }
    if (pos.empty() || pos.back().se != n - 1) pos.push_back({l, n-1});
    // dbg(pos);

    int res = -1e18;
    for (int i = 0; i < sz(pos); ++i) {
        int lmin = 0, lmax = -1e18, sum = 0;
        for (int j = pos[i].fi; j <= pos[i].se; ++j) {
            sum += a[j];
            lmax = max(lmax, sum - lmin);
            lmin = min(lmin, sum);
        }
        res = max(res, lmax);
    }
