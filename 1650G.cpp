...
void solve()
{
    ...
    while (!q.empty()) {
        ...
        if (dist[u] > minDist + 1) continue;

        if (dist[u] >= minDist && dist[u] <= minDist + 1 && u == t) {
            res = (res + dp[vis[u]][u]) % mod;
            continue;
        }

        for (auto &v : adj[u]) {
            if (dist[u] <= dist[v]) {
                dp[vis[v]][v] = (dp[vis[v]][v] + dp[vis[u]][u]) % mod;
                if (vis[v] == 0 || (vis[v] == 1 && dist[v] == minDist - 1)) {
                    vis[v]++;
                    q.push(v);
                }
            }
